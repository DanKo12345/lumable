...
![Предпросмотр экрана и музыки в LumaBLE 0.4.4](docs/images/lumable-screen-preview-0.4.4-dark.png)

*Демонстрационные цвета предпросмотра. Лента и звуковое устройство не подключались.*

## Возможности

- Управление RGB, яркостью, цветовой температурой и питанием.
- Сцены, быстрые режимы, встроенные и программные эффекты.
- Режим «Экран + музыка»: экран задаёт цвет, а музыка добавляет яркость и силу ударов.
- Отдельная синхронизация только с экраном или только с музыкой.
- Автоматизации по времени, активному приложению, простою, подключению ленты и событиям Windows.
- Быстрые действия в трее и необязательные глобальные горячие клавиши.
- Перенос сцен, правил, групп и настроек через безопасную резервную копию.
...
### Распознаётся, управление пока не поддерживается

- **BanlanX SP630E** — LumaBLE опознаёт его по эфиру, но команд не отправляет: протокол управления
  ещё не проверен на реальном устройстве. Выберите устройство и нажмите **Проверить** — это чтение
  без записи. Затем сохраните **отчёт BLE-сканирования** в диагностике и приложите к
  [issue #2](https://github.com/DanKo12345/lumable/issues/2).

### Как сообщить о неизвестном контроллере

Выполните поиск, затем **Диагностика → Отчёт BLE-сканирования**. Файл записывает то, что передавали
все устройства рядом, включая отфильтрованные из видимого списка — именно туда попадает
нераспознанный контроллер. Внутри переданных данных могут быть технические идентификаторы, поэтому
просмотрите файл перед публикацией.
